...
void load_player(t_data *data)
{
    ...
    data->player.front.img = mlx_xpm_file_to_image(data->mlx_ptr, "assets/player/static_img/player_right.xpm", &data->player.front.width, &data->player.front.height);
    if (!data->player.front.img)
        exit_error(data, "LOAD_SPRITES: ERROR LOADING PLAYER FRONT IMAGE\n");
    data->player.back.img = mlx_xpm_file_to_image(data->mlx_ptr, "assets/player/static_img/player_right.xpm", &data->player.back.width, &data->player.back.height);
